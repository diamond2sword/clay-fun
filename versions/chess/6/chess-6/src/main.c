...
uint64_t Mask_SlidingAttacks(uint64_t mask_occupied, uint8_t index, int32_t ray_count, ...);
...
bool Piece_IsMove(int8_t index);
...
uint64_t Mask_Moves(Bitboards_All bitboardSet, uint8_t index, PieceSideIndex side, PieceTypeIndex type)
{
	...
	case PIECE_TYPE_INDEX_PAWN: {
		...
		uint64_t mask_singleForward = ~mask_occupied & MASK_INDEX_UNBOUND(index + offsetForward);
		uint64_t mask_captures = Mask_Cols(2, col - 1, col + 1) & MASK_ROW(row - offsetSign) & (mask_enemy | mask_enPassantTarget);
		uint64_t mask_doubleForward = (mask_asUnmovedPawn && mask_singleForward) ? ~mask_occupied & MASK_INDEX_UNBOUND(index + offsetForward * 2) : MASK_EMPTY;
		return mask_singleForward | mask_doubleForward | mask_captures;
	} case PIECE_TYPE_INDEX_ROOK: {
		uint64_t mask_attacks = Mask_SlidingAttacks(mask_occupied, index, 4, 
			PIECE_RAY_INDEX_NORTH,
			PIECE_RAY_INDEX_SOUTH,
			PIECE_RAY_INDEX_WEST,
			PIECE_RAY_INDEX_EAST
		);
		return mask_attacks & (mask_enemy | ~mask_occupied);
		...
		uint64_t mask_attacks = Mask_SlidingAttacks(mask_occupied, index, 8, 
			PIECE_RAY_INDEX_NORTH,
			PIECE_RAY_INDEX_SOUTH,
			PIECE_RAY_INDEX_WEST,
			PIECE_RAY_INDEX_EAST,
		...
		return mask_attacks & (mask_enemy | ~mask_occupied);
		// north
		//uint8_t col_blocker = 7 - Index_AsCol(index_blocker);
		//uint8_t row_blocker = 7 - Index_AsRow(index_blocker);
		//uint64_t mask_ray_blocked = MASK_ROW(Index_AsRow(index_blocker)) & MASK_COL(Index_AsCol(index_blocker));
		/*
		...
		uint64_t mask_attacks = MASK_EMPTY;
		{
			uint8_t ray = PIECE_RAY_INDEX_SOUTH;
			uint64_t mask_rays_this	= MAP_DIRECTION_TO_RAY_MASK[ray][index];
			mask_attacks |= mask_rays_this;
			uint64_t mask_matches = mask_rays_this & mask_occupied;
			if (mask_matches)
			{
				uint8_t index_blocker = BitScanForward(mask_matches);
				mask_attacks &= ~MAP_DIRECTION_TO_RAY_MASK[ray][index_blocker];
			}
		}
		{
			uint8_t ray = PIECE_RAY_INDEX_NORTH;
			uint64_t mask_rays_this	= MAP_DIRECTION_TO_RAY_MASK[ray][index];
			mask_attacks |= mask_rays_this;
			uint64_t mask_matches = mask_rays_this & mask_occupied;
			if (mask_matches)
			{
				uint8_t index_blocker = BitScanReverse(mask_matches);
				mask_attacks &= ~MAP_DIRECTION_TO_RAY_MASK[ray][index_blocker];
			}
		}
		*/
		//return Direct_Rows(ACTIVE_SIDE, Mask_Rows(Index_AsRow(index_blocker) + 1, 7, 6, 5, 4, 3, 2, 1, 0));
		//return MASK_INDEX(index_blocker);
		//return MAP_DIRECTION_TO_RAY_MASK[PIECE_RAY_INDEX_SOUTH][24];
...
bool Piece_IsMove(int8_t index)
{
	return MASK_INDEX_SIGNED(index) & MOVES_BITBOARD;
}
...
void BoardClick_Phase_Reset_AutoSelect()
{
	const bool selected_isOtherAlly = CELL_INDEX != SRC_CELL_INDEX;
	// reset click phase
	BoardClick_Phase_Reset();
	// skip src phase if selected is movable
	if (selected_isOtherAlly) BoardClick_Phase_SelectSource();
}
