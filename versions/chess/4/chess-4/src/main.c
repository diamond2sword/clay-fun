...
uint64_t Mask_Moves(Bitboards_All bitboardSet, uint8_t index, PieceSideIndex side, PieceTypeIndex type)
{
	...
	switch (Piece_GetTypeIndex(bitboardSet, index)) {
...
uint64_t PROMOTION_OPTS_BITBOARD = MASK_EMPTY;
uint64_t Mask_PromotionOptions(Bitboards_All bitboardSet, uint64_t index, Move move)
{
	const uint64_t MASK_ROWS_0_TO_3 = Mask_Rows(4, 0, 1, 2, 3);
	uint64_t mask_options = Direct_Rows(move.src.side, MASK_ROWS_0_TO_3 & MASK_COL(Index_AsCol(move.dst.index)));
	return mask_options;
}
...
void BoardClick_Phase_MakeMove()
{
	...
	}
	{
		...
		if (RECENT_MOVE.src.type == PIECE_TYPE_INDEX_PAWN)
		{
			...
			else if (RECENT_MOVE.dst.index == EN_PASSANT_TARGET_INDEX)
			{
...
const float subrectangleWidth = 270.f/6;
const float subrectangleHeight = 90.f/2;
const float pieceWidthRatio = 1.f;
...
void BoardPromotionPopup()
{
	...
	for (uint8_t optIndex = 0; optIndex < 4; optIndex++)
	{
		...
		CLAY(CLAY_LAYOUT(SIZING_FIXED(BOARD_CELL_WIDTH, BOARD_CELL_WIDTH)),
			CLAY_RECTANGLE(Color_AsFaded(colorWhite)),
			CLAY_FLOATING()
		)
		CLAY(CLAY_LAYOUT(SIZING_FIXED(PIECE_WIDTH, PIECE_WIDTH)),
			CLAY_FLOATING({.attachment={CLAY_ATTACH_POINT_CENTER_CENTER, CLAY_ATTACH_POINT_CENTER_CENTER}}),
			CLAY_IMAGE({.sourceURL=CLAY_STRING("/clay/images/pieces.svg"),
				.subrectangle={
					subrectangleWidth * MAP_PROMOTION_OPT_TO_TYPE[optIndex],
					subrectangleHeight * RECENT_MOVE.src.side,
					subrectangleWidth,
					subrectangleHeight
				}
			})
...
{
	{
		...
		PIECE_WIDTH = BOARD_CELL_WIDTH * pieceWidthRatio;
	}
    Clay_BeginLayout();
	{
		BoardBackground();
